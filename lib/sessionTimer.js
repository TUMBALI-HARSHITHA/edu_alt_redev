...
export function initCourseLearningTimer(user, isLearningPage = false) {
  const uid = user?.uid;
  ...
  if (isLearningPage) {
    ...
    learningTimerInterval = setInterval(() => {
      ...
      if (!isTabHidden && !isInactive) {
        ...
        if (user && courseLearningSeconds % 30 === 0) {
          try {
            const userRef = doc(db, "users", user.uid);
            setDoc(userRef, {
...
export function getCourseLearningSeconds(uid, userMetrics = [], firestoreCourseTime = 0) {
  ...
  return Math.max(storedSeconds, metricsSeconds, firestoreCourseTime || 0);
}
