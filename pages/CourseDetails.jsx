...
const CourseDetails = () => {
  ...
  useEffect(() => {
    const fetchCourseAndEnrollment = async (currentUser) => {
      if (!courseId) return;
      try {
        let found = false;
        ...
        if (!found) {
          ...
          if (idx !== -1) {
            let base = { id: `pc-${idx}`, ...PLATFORM_COURSES[idx] };
            try {
        ...
        if (currentUser) {
          ...
          if (!querySnapshot.empty) {
            const sorted = querySnapshot.docs.map((d) => ({ id: d.id, ...d.data() })).sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
          ...
          const courseApps = myAppSnap.docs.map((d) => ({ id: d.id, ...d.data() })).filter((app) => app.qualification === courseId);
          ...
          const loadedMentors = approvedForCourse.map((doc2) => {
  ...
  const finalizeEnrollment = async (plan = "full") => {
    ...
    try {
      ...
      const newEnrollment = {
        ...
        mentorId: selectedMentor || void 0,
...
The Edu-Alt-Tech Team`
      ...
      const options = {
        ...
        handler: async function(response) {
          try {
            ...
            const verifyData = await resVerify.json();
            if (verifyData.success) {
              await finalizeEnrollment("first_class");
            } else {
              throw new Error(verifyData.error || "Invalid Security Signature");
            }
          } catch (e) {
  ...
  return <div className="min-h-screen pt-24 sm:pt-32 pb-24 sm:pb-32 px-4 sm:px-6 [#020617] relative overflow-hidden">
 ...
 <motion.div
    ...
    ref={contentRef}
  >
  <Link to="/courses" className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-900 transition-colors mb-6 sm:mb-10 font-medium text-sm sm:text-base"> <ArrowLeft className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Back to Courses
  </Link>

 {
    /* Hero Card */
  }
  <div className="/90 backdrop-blur-2xl rounded-[1.75rem] sm:rounded-[2.5rem] p-5 sm:p-10 md:p-14 border border-slate-200/50 shadow-2xl mb-6 sm:mb-8"> <div className="flex flex-col md:flex-row justify-between gap-4 sm:gap-6 mb-6 sm:mb-8">
  <div>
  ...
  <h1 className="text-[1.6rem] leading-[1.1] sm:text-4xl md:text-5xl font-black text-slate-900 mb-3 sm:mb-4 tracking-tight">{course.title}</h1> <p className="text-sm sm:text-base md:text-xl text-slate-600 font-medium leading-relaxed sm:leading-normal">{course.description}</p>
  </div>
  <div className="flex flex-row md:flex-col items-center md:items-end gap-3 md:gap-0 flex-shrink-0"> <span className="text-xl sm:text-2xl md:text-3xl font-bold text-emerald-600 md:mb-2">
 ...
 <button
  ...
  >
 <div className="flex items-center gap-4"> <div className="w-10 h-10 bg-emerald-500/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
 <Sparkles className="w-5 h-5 text-emerald-500" />
 </div>
 <div className="text-left"> <span className="block font-bold text-slate-900 text-sm">Ask AI about this course</span>
 ...
 <Sparkles className="w-5 h-5 text-emerald-400 group-hover:rotate-12 transition-transform" />
  ...
  <div className="p-5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl border border-slate-100 "> {enrollment ? <div className="flex flex-col items-center text-center">
  {enrollment.role === "student" ? enrollment.paymentStatus === "pending" ? <>
  <AlertCircle className="w-12 h-12 sm:w-16 sm:h-16 text-blue-500 mb-3 sm:mb-4" /> <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2">Mentor Assigned!</h2>
  ...
  A mentor is ready to teach you. Please complete the payment to start learning.
   ...
   <CheckCircle2 className="w-12 h-12 sm:w-16 sm:h-16 text-emerald-500 mb-3 sm:mb-4" /> <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2">You are enrolled!</h2>
   <Link to="/dashboard" className="text-emerald-600 hover:underline mt-2 font-medium flex items-center gap-1 text-sm sm:text-base"> Go to Dashboard <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
   </Link>
   <p className="text-xs text-slate-400 mt-4">Want to learn from a different mentor? Select one below and enroll again.</p> <div className="w-full mt-4 space-y-4">
    {mentors.map((m) => <div
    key={m.userId}
  ...
  >
      <div className="flex justify-between items-start mb-2 gap-2"> <p className="font-bold text-slate-900 text-base sm:text-lg">{m.name}</p>
        {selectedMentor === m.userId && <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500 shrink-0" />}
      </div>
      <p className="text-xs sm:text-sm text-slate-500 mb-1 leading-snug"><strong>Experience:</strong> {m.experience}</p> {m.highestQualification && <p className="text-xs sm:text-sm text-slate-500 mb-1 leading-snug"><strong>Qualification:</strong> {m.highestQualification}</p>}
  ...
  >
      ...
      {course?.price && course.price > 0 && <button
  ...
  <CheckCircle2 className="w-12 h-12 sm:w-16 sm:h-16 text-purple-500 mb-3 sm:mb-4" /> <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2">Teacher Application Submitted</h2>
  <p className="text-sm sm:text-base text-slate-600 max-w-md mx-auto mb-5 sm:mb-6">
  You have applied to teach this course. Check your dashboard for appointment updates.
  </p>
  <Link to="/dashboard" className="text-purple-600 hover:underline mt-2 font-medium flex items-center gap-1 text-sm sm:text-base"> View Schedule <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
  </Link>
 </>}
 </div> : <div className="flex flex-col gap-6"> <div className="flex-1  border border-slate-200 p-5 sm:p-6 rounded-2xl shadow-sm">
  <h3 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 flex items-center gap-2"><BookOpen className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500" /> Join as a Student</h3> {mentors.length === 0 ? <div className="p-4  rounded-xl text-center">
 <p className="text-slate-500 font-medium">No active mentors available right now.</p> <p className="text-sm mt-1 text-slate-400">Please wait for a mentor to be approved to teach.</p>
 ...
 {mentors.map((m) => <div
  ...
  <p className="text-xs sm:text-sm text-slate-500 mb-1 leading-snug"><strong>Experience:</strong> {m.experience}</p> {m.highestQualification && <p className="text-xs sm:text-sm text-slate-500 mb-1 leading-snug"><strong>Qualification:</strong> {m.highestQualification}</p>}
  <p className="text-xs sm:text-sm text-slate-500 leading-snug"><strong>Skills:</strong> {m.skills}</p> {m.message && <p className="text-xs text-slate-400 mt-2 italic">"{m.message}"</p>}
 
 {m.proposedPath && m.proposedPath.length > 0 && <div className="mt-3 pt-3 border-t border-slate-100 "> <p className="text-xs font-bold text-slate-700 mb-1">Proposed Curriculum:</p>
 <ul className="list-disc list-inside text-xs text-slate-500 space-y-1"> {m.proposedPath.map((pathItem, idx) => <li key={idx} className="truncate">{pathItem}</li>)}
 </ul>
 </div>}
 </div>)}
  ...
  <div className="flex-1 /80 backdrop-blur border border-purple-200/50 p-5 sm:p-6 rounded-2xl flex flex-col justify-center items-center text-center shadow-sm"> <h3 className="text-base sm:text-lg font-bold mb-2 flex items-center justify-center gap-2"><Users className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500" /> Teach this Course</h3> <p className="text-slate-500 text-sm mb-4 max-w-sm">
 Are you qualified to teach this subject? Apply to become a mentor and start teaching students securely.
 </p>
 {myAppStatus === "pending" || myAppStatus === "scheduled" ? <button disabled className="w-full max-w-xs bg-slate-200 text-slate-500 font-bold py-3 px-8 rounded-xl cursor-not-allowed">
 Application Under Review
 </button> : myAppStatus === "rejected" ? <button
    onClick={handleApplyToTeach}
    className="w-full max-w-xs text-purple-600 border-2 border-purple-200 hover:border-purple-500 font-bold py-3 px-8 rounded-xl transition-colors shadow-sm"
  >
 Re-apply to Teach
 </button> : myAppStatus === "approved" ? <button disabled className="w-full max-w-xs bg-emerald-50 text-emerald-600 font-bold py-3 px-8 rounded-xl cursor-not-allowed border border-emerald-200 ">
 You are a Mentor
 </button> : <button
    onClick={handleApplyToTeach}
    className="w-full max-w-xs text-purple-600 border-2 border-purple-200 hover:border-purple-500 font-bold py-3 px-8 rounded-xl transition-colors shadow-sm"
