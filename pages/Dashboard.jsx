...
const extractMeetingLink = (message, explicitLink) => {
  if (explicitLink) return explicitLink;
  if (!message) return null;
  const match = message.match(/\[Interview Link:\s*([^\]]+)\]/);
  return match ? match[1] ?? null : null;
};
const extractMeetingDate = (message, explicitDate) => {
...
const ProgressRing = ({
  ...
  return <div className="relative shrink-0" style={{ width: size, height: size }}> <svg width={size} height={size} className="transform -rotate-90">
        <circle
    cx={size / 2}
    cy={size / 2}
    r={radius}
...
const Sparkline = () => {
  return <svg className="w-12 h-6 text-indigo-500 shrink-0" viewBox="0 0 60 20">
      <path
...
const Dashboard = () => {
  ...
  const [teachingEnrollments, setTeachingEnrollments] = useState([]);
  const [myApplications, setMyApplications] = useState([]);
  const [chatMessages, setChatMessages] = useState([]);
  const [chatInput, setChatInput] = useState("");
  const [sendingMessage, setSendingMessage] = useState(false);
  const [, setLoading] = useState(true);
  const [, setRejectionCounts] = useState({});
  ...
  const [firstClassLockedCourse, setFirstClassLockedCourse] = useState(null);
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        PLATFORM_COURSES.forEach((pc, i) => {
          const id = `pc-${i}`;
          if (!coursesMap.has(id)) coursesMap.set(id, { id, ...pc });
        });
          ...
          (snap) => {
            ...
            if (!cancelled) {
              ...
              const lockedFc = studentEnr.find((e) => {
                ...
                return Date.now() - created.getTime() > 24 * 60 * 60 * 1e3;
              });
        ...
        const unsubPractice = onSnapshot(
          ...
          (snap) => {
            ...
            if (!cancelled) {
              ...
              const lc = history.filter((h) => h.practiceType === "leetcode").length;
              const eng = history.filter((h) => h.practiceType === "english").length;
              setLeetcodeCount(lc);
              setEnglishCount(eng);
            }
          }
  ...
  useEffect(() => {
    ...
    const enrolledCourseIds2 = enrollments.map((e) => e.courseId).filter(Boolean);
    if (enrolledCourseIds2.length === 0) return;
    const timestamps = getLastReadTimestamps(user.uid);
    const unsubChatCounts = onSnapshot(
      query(
        collection(db, "course_chat_messages"),
        where("courseId", "in", enrolledCourseIds2)
      ),
      (snap) => {
        const counts = {};
        ...
        enrollments.forEach((enr) => {
          ...
          const unreadCount = allMsgs.filter((m) => m.courseId === enr.courseId && m.createdAt > lastRead).length;
          if (unreadCount > 0) {
  ...
  useEffect(() => {
    ...
    const enrolledCourseIds2 = enrollments.map((e) => e.courseId).filter(Boolean);
    if (enrolledCourseIds2.length === 0) {
      setUpcomingClasses([]);
      return;
    }
    const unsubClasses = onSnapshot(
      ...
      (snap) => {
        ...
        const classes = snap.docs.map((d) => ({ id: d.id, ...d.data() })).filter((cls) => !cls.scheduledAt || cls.scheduledAt >= nowStr).slice(0, 10);
  ...
  const renderSVGChart = () => {
    ...
    if (chartData.length === 0) {
      return <div className="flex flex-col items-center justify-center py-12 text-slate-400 text-xs text-center border border-dashed border-slate-200 rounded-3xl"> <BarChart3 className="w-10 h-10 mb-3 opacity-30 text-emerald-500" />
          <span className="font-semibold text-slate-500">Analytics Sandbox Empty</span> <span className="text-[10px] mt-0.5 max-w-[200px] text-slate-400">Metrics will dynamically plot once you begin learning</span>
        </div>;
    }
    const width = 500;
    const height = 240;
    ...
    return <div className="w-full h-full min-h-[220px]"> <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full text-slate-600 font-sans">
          {
      /* Y Axis Grid lines */
    }
      ...
      return <g key={idx} className="group">
                {
...
Module Progress: ${data.progress}%`}</title>
  ...
  return <div className="min-h-screen pt-40 md:pt-24 lg:pt-28 pb-16 sm:pb-20 px-3 sm:px-4 md:px-8 "> <div className="max-w-7xl mx-auto">
  ...
  >
          <div className="absolute top-0 right-0 p-8 opacity-10"><GraduationCap className="w-64 h-64 text-white" /></div> <div className="absolute -left-12 -top-12 w-48 h-48 rounded-full /10 blur-[80px]" />
          <div className="absolute right-12 bottom-0 w-64 h-64 rounded-full bg-teal-400/25 blur-[100px]" /> <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="flex items-center gap-4"> <div className="w-16 h-16 rounded-2xl /20 border border-white/30 p-0.5 overflow-hidden shadow-inner flex items-center justify-center font-black text-xl text-white">
                {userProfile?.profilePic ? <img src={userProfile.profilePic} loading="lazy" decoding="async" alt="" className="w-full h-full object-cover rounded-xl" /> : userProfile?.name?.charAt(0) || "U"}
              </div>
              <div>
                <h1 className="text-xl sm:text-2xl md:text-3xl font-black tracking-tight !text-white font-display"> {getGreeting()}, {userProfile?.name || "Learner"}
                </h1>
                <div className="flex items-center gap-2 mt-1.5"> <span className="px-2.5 py-0.5 /20 border border-white/10 !text-white rounded-lg text-[10px] font-extrabold uppercase tracking-wider">
                    {userProfile?.role || "Student"}
                  </span>
                  {averageConsistency >= 70 && <span className="flex items-center gap-1 px-2.5 py-0.5 bg-amber-400/25 border border-amber-400/35 !text-amber-200 rounded-lg text-[10px] font-extrabold uppercase tracking-wider"> <Flame className="w-3 h-3 text-amber-300 fill-amber-300" /> Super Active
                    </span>}
                </div>
              </div>
            </div>

            <div className="flex items-center gap-3 flex-wrap"> <span className="inline-flex items-center gap-2 px-4 py-2 /15 border border-white/10 rounded-full text-xs font-bold !text-white backdrop-blur-md shadow-sm">
                <BookOpen className="w-3.5 h-3.5 text-white" /> {enrollments.length} Enrolled
              </span>
              {totalTeaching > 0 && <span className="inline-flex items-center gap-2 px-4 py-2 /15 border border-white/10 rounded-full text-xs font-bold !text-white backdrop-blur-md shadow-sm"> <Users className="w-3.5 h-3.5 text-white" /> {totalTeaching} Teaching
                </span>}
              <span className="inline-flex items-center gap-2 px-4 py-2 /15 border border-white/10 rounded-full text-xs font-bold !text-white backdrop-blur-md shadow-sm"> <Download className="w-3.5 h-3.5 text-white" /> {resourceCount} resources
              </span>
    ...
    return <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="mb-6"> <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 rounded-3xl p-5 text-white shadow-xl shadow-indigo-600/10 relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="absolute top-0 right-0 p-3 opacity-5"><Video className="w-32 h-32" /></div> <div className="relative z-10 flex items-start gap-4">
                  <div className="w-12 h-12 rounded-2xl /20 flex items-center justify-center shrink-0 shadow-lg backdrop-blur-sm"><Video className="w-6 h-6" /></div>
                  <div>
                    <div className="flex items-center gap-2 mb-1"> <span className="px-2.5 py-0.5 /25 rounded-full text-[9px] font-black uppercase tracking-wider">Interview Scheduled</span>
                    </div>
                    <h2 className="text-base sm:text-lg font-black tracking-tight !text-white">Teacher Application Review</h2> <p className="text-indigo-100 text-xs mt-0.5">{scheduledApp.courseTitle}{mDate ? <> — <strong>{new Date(mDate).toLocaleString()}</strong></> : ""}</p>
                  </div>
                </div>
                <a
      ...
      target="_blank"
      rel="noreferrer"
      className="relative z-10 shrink-0 inline-flex items-center justify-center gap-2 px-6 py-3 text-indigo-700 font-extrabold rounded-xl hover: hover:scale-[1.02] active:scale-95 transition-all shadow-md text-sm"
    >
    ...
    /* ── Next Steps Widget ── */
        ...
        {!scheduledApp && nextSteps.length > 0 && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-6"> <div className="border border-slate-200 rounded-3xl p-5 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 right-0 p-2 opacity-5"><Lightbulb className="w-24 h-24 text-emerald-500" /></div> <div className="relative z-10 flex items-start gap-4">
                <div className="w-10 h-10 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-500 shrink-0"><Lightbulb className="w-5 h-5" /></div> <div className="flex-1">
                  <h2 className="text-sm font-black text-slate-800 tracking-tight mb-2">Next Steps</h2> <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {nextSteps.slice(0, 2).map((step, i) => <div key={i} className="flex items-center gap-2.5 p-2 rounded-xl border border-slate-100 text-slate-705 text-xs">
                        <span className="w-6 h-6 rounded-lg bg-emerald-100 text-emerald-700 flex items-center justify-center text-[10px] font-black shrink-0">{i + 1}</span> <span className="font-semibold leading-tight text-slate-655">{step}</span>
                      </div>)}
                  </div>
                </div>
              </div>
    ...
    /* ── First Class Trial Lock Panel ── */
        ...
        {firstClassLocked && <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="mb-6"> <div className="rounded-[2.5rem] border-2 border-rose-200 shadow-xl overflow-hidden">
              <div className="bg-gradient-to-r from-rose-500 to-rose-700 p-8 sm:p-12 text-center relative"> <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,255,255,0.15),transparent_70%)]" />
                <div className="relative z-10"> <div className="w-20 h-20 mx-auto mb-5 /20 rounded-3xl flex items-center justify-center backdrop-blur-sm">
                    <Lock className="w-10 h-10 text-white" />
                  </div>
                  <h2 className="text-2xl sm:text-3xl font-black !text-white tracking-tight mb-2">Trial Period Ended</h2> <p className="text-rose-100 text-sm sm:text-base max-w-lg mx-auto leading-relaxed">
              ...
              <div className="p-6 text-center bg-rose-50/50">
                <button
    onClick={() => {
      if (firstClassLockedCourse) navigate(`/classroom/${firstClassLockedCourse}`);
    }}
  ...
  >
    ...
    className="border border-slate-200 rounded-[2rem] p-6 shadow-sm"
  >
              <div className="flex items-center justify-between mb-5">
                <div>
                  <h2 className="text-base sm:text-lg font-black text-slate-800 flex items-center gap-2"> <GraduationCap className="w-5 h-5 text-emerald-500" /> Active Classrooms
                  </h2>
                  <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">Resume your studies</p>
                </div>
                {enrollments.length > 0 && <Link to="/courses" className="text-xs font-black text-emerald-600 hover:text-emerald-500 transition-colors">
                    Find More Courses
                  </Link>}
              </div>

              {enrollments.length === 0 ? <div className="border border-slate-200 border-dashed rounded-3xl p-8 text-center"> <div className="w-16 h-16 rounded-3xl bg-emerald-50 text-emerald-500 flex items-center justify-center mx-auto mb-4">
                    <BookOpen className="w-8 h-8" />
                  </div>
                  <h3 className="text-lg font-bold text-slate-800 mb-1">Your learning tray is empty</h3> <p className="text-xs text-slate-400 max-w-sm mx-auto mb-6">Unlock interactive modules, coding problems, and personalized mentorship by enrolling in a course.</p>
                  <Link to="/courses" className="inline-flex items-center gap-2 px-6 py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold text-xs transition-colors shadow-md shadow-emerald-600/10"> Explore courses catalog <ArrowRight className="w-4 h-4" />
                  </Link>
    ...
    return <div
      ...
      className="group relative hover: border border-slate-200/80 hover:border-emerald-500 rounded-2xl p-4 hover:shadow-md cursor-pointer transition-all duration-300"
    >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4"> <div className="flex items-center gap-3.5 min-w-0">
                            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-400 to-emerald-600 text-white flex items-center justify-center font-black text-base shadow-md shrink-0"> {enr.courseData?.title?.charAt(0) || "C"}
                            </div>
                            <div className="min-w-0"> <span className="px-2 py-0.5 bg-slate-200 text-slate-600 rounded text-[9px] font-black uppercase tracking-wider">
                                {enr.courseData?.category || "General"}
                              </span>
                              <h3 className="text-sm font-black text-slate-800 truncate mt-1 group-hover:text-emerald-600 transition-colors"> {enr.courseData?.title || "Course Details"}
                              </h3>
                            </div>
                          </div>

                          <div className="flex items-center gap-4 self-end sm:self-auto"> <div className="hidden sm:block text-right">
                              <span className="text-[10px] font-extrabold text-slate-400 block uppercase">Completed</span> <span className="text-xs font-bold text-slate-700">{completed}/{total} Modules</span>
                            </div>

                            {(chatUnreadCounts[enr.courseId] ?? 0) > 0 && <span className="px-2 py-0.5 bg-emerald-500 text-white text-[9px] font-extrabold rounded-full shrink-0 animate-pulse">
      ...
      /* Interactive Course Progress Bar */
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-3"> <div className="flex-1 h-2 bg-slate-200/60 rounded-full overflow-hidden">
                            <div
  ...
  >
              <div>
                <h3 className="text-base sm:text-lg font-black text-slate-850 flex items-center gap-2"> <BarChart3 className="w-5 h-5 text-emerald-500" /> Study Dynamics
                </h3>
              ...
              {enrollments.length > 0 && <div className="mt-4 flex items-center justify-center gap-4 text-[10px] font-extrabold text-slate-400"> <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-emerald-500 inline-block" /> Completion Progress (%)</span>
                </div>}
            </motion.div>
    ...
    /* Practice timeline activity log */
            ...
            {practiceHistory.length > 0 && <motion.div
    initial={{ opacity: 0, y: 15 }}
    animate={{ opacity: 1, y: 0 }}
  ...
  >
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="text-base sm:text-lg font-black text-slate-800 flex items-center gap-2"> <History className="w-5 h-5 text-indigo-500" /> Recent Activities
                    </h2>
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">Practice history log</p>
                  </div>
                </div>
                <div className="max-h-60 overflow-y-auto pr-1 custom-scrollbar"> <div className="relative pl-6 border-l border-slate-100 space-y-4 py-2">
                    {practiceHistory.slice(0, 10).map((h) => <div key={h.id} className="relative"> <div className={`absolute -left-[31px] top-1.5 w-2.5 h-2.5 rounded-full border-2 border-white shadow-sm ${h.practiceType === "leetcode" ? "bg-blue-500" : "bg-indigo-500"}`} />
                        <div className="flex items-center justify-between gap-4">
                          <div>
                            <span className="text-xs font-bold text-slate-800 block leading-tight">{h.itemTitle}</span> <span className="text-[10px] font-semibold text-slate-400 block mt-0.5">
    ...
    className="border border-slate-200 rounded-[2rem] p-5 shadow-sm"
  >
              <div className="flex items-center gap-3.5"> <div className="w-14 h-14 bg-gradient-to-br from-emerald-400 to-indigo-500 text-white rounded-2xl flex items-center justify-center font-black text-xl shadow-md shrink-0 overflow-hidden">
                ...
                <div className="min-w-0"> <h3 className="font-black text-slate-800 text-sm truncate">{userProfile?.name || "Student Account"}</h3>
                  <p className="text-[11px] text-slate-400 font-semibold truncate mt-0.5">{userProfile?.email}</p> <span className="inline-block mt-1 px-2.5 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-lg text-[9px] font-black uppercase tracking-wider">
                    {userProfile?.role || "Student"}
                  </span>
                </div>
              </div>
              <Link to="/profile" className="flex items-center justify-between w-full mt-4 px-4 py-3 hover:bg-slate-100 rounded-xl text-xs font-black text-slate-600 hover:text-slate-900 transition-colors">
    ...
    /* Live Upcoming classes */
            ...
            {upcomingClasses.length > 0 && <motion.div
    initial={{ opacity: 0, y: 15 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ delay: 0.12 }}
    className="border border-slate-200 rounded-[2rem] p-5 shadow-sm"
  >
                <div className="mb-3"> <h3 className="font-black text-slate-800 text-sm flex items-center gap-2">
                ...
                <div className="space-y-3"> {upcomingClasses.map((cls) => <div key={cls.id} className="p-3.5 rounded-2xl bg-indigo-50/50 border border-indigo-100/40">
                      <div className="flex items-start gap-3"> <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-400 to-indigo-600 text-white flex items-center justify-center shrink-0 shadow-md">
                          <Calendar className="w-4.5 h-4.5" />
                        </div>
                        <div className="flex-1 min-w-0"> <h4 className="text-xs font-black text-slate-800 leading-snug">{cls.title}</h4>
                          <p className="text-[10px] text-slate-500 font-medium leading-normal mt-0.5">{cls.description}</p> <div className="flex items-center gap-2 mt-2 text-[10px] font-black text-indigo-700">
                            <span>
  ...
  >
    ...
    className="border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm flex flex-col"
  >
              <div className="p-5 border-b border-slate-100 "> <h3 className="font-black text-slate-800 text-sm flex items-center gap-2">
    ...
    /* Message tray */
              ...
              <div className="h-60 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                {chatMessages.length === 0 ? <div className="text-center py-8"> <MessageSquare className="w-10 h-10 text-slate-300 mx-auto mb-2 opacity-50" />
                    <p className="text-slate-500 font-bold text-xs">Sandbox Help Room</p> <p className="text-[10px] text-slate-400 mt-0.5">Inquire about curriculum options below</p>
                  </div> : chatMessages.map((msg) => <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[85%] p-3 rounded-2xl shadow-sm text-xs font-semibold leading-relaxed ${msg.role === "user" ? "bg-emerald-600 text-white rounded-br-none" : "bg-white text-slate-800 border border-slate-200/60 rounded-bl-none"}`}>
                        <p>{msg.content}</p>
                        <span className={`text-[8px] font-bold block mt-1 text-right ${msg.role === "user" ? "text-emerald-200" : "text-slate-400"}`}>
    ...
    /* Input field */
              ...
              <div className="p-3 border-t border-slate-100 "> <div className="flex gap-2">
                  <input
    value={chatInput}
    ...
    className="flex-grow px-3 py-2 rounded-xl outline-none font-semibold text-xs border border-transparent focus:border-emerald-500 focus: transition-all"
  />
                  <button
  ...
  >
    ...
    className="border border-slate-200 rounded-[2rem] p-5 shadow-sm"
  >
              <h3 className="font-black text-slate-800 text-sm mb-3">Resources & Shortcuts</h3> <div className="grid grid-cols-2 gap-2">
                <Link to="/practice" className="flex flex-col items-center justify-center gap-1.5 p-3 rounded-2xl bg-indigo-50/50 border border-indigo-100/40 hover:shadow-md transition-all duration-300 group"> <Code2 className="w-5 h-5 text-indigo-600 group-hover:scale-110 transition-transform" />
                  <span className="text-[10px] font-black text-slate-700">Coding practice</span>
                </Link>
                <Link to="/practice" className="flex flex-col items-center justify-center gap-1.5 p-3 rounded-2xl bg-emerald-50/50 border border-emerald-100/40 hover:shadow-md transition-all duration-300 group"> <BookOpen className="w-5 h-5 text-emerald-600 group-hover:scale-110 transition-transform" />
                  <span className="text-[10px] font-black text-slate-700">English skills</span>
                </Link>
                <Link to="/resources" className="flex flex-col items-center justify-center gap-1.5 p-3 rounded-2xl bg-amber-50/50 border border-amber-100/40 hover:shadow-md transition-all duration-300 group"> <Download className="w-5 h-5 text-amber-600 group-hover:scale-110 transition-transform" />
                  <span className="text-[10px] font-black text-slate-700">Files library</span>
                </Link>
                <Link to="/courses" className="flex flex-col items-center justify-center gap-1.5 p-3 rounded-2xl bg-rose-50/50 border border-rose-100/40 hover:shadow-md transition-all duration-300 group"> <GraduationCap className="w-5 h-5 text-rose-600 group-hover:scale-110 transition-transform" />
    ...
    /* Mentorship shortcut application */
              ...
              <Link to="/teacher-application" className="mt-3 block w-full p-4 rounded-2xl bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-700 hover:from-indigo-500 hover:to-purple-500 text-white shadow-md hover:shadow-lg transition-all duration-300 group relative overflow-hidden"> <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.12),transparent_60%)] pointer-events-none" />
                <div className="relative z-10 flex items-center gap-3.5"> <div className="w-10 h-10 rounded-xl /20 flex items-center justify-center backdrop-blur-sm group-hover:scale-105 transition-transform shrink-0">
                    <Award className="w-5 h-5 text-white" />
                  </div>
                  <div className="flex-grow min-w-0"> <p className="font-black text-xs tracking-tight">Become a Teacher</p>
    ...
    /* Applications history tracker */
            ...
            {totalApplications > 0 && <motion.div
    initial={{ opacity: 0, y: 15 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ delay: 0.18 }}
    className="border border-slate-200 rounded-[2rem] p-5 shadow-sm"
  >
                <h3 className="font-black text-slate-850 text-sm mb-3 flex items-center gap-2"> <FileText className="w-4 h-4 text-amber-500" /> Teaching Review Desk
                </h3>
                <div className="space-y-2"> {pendingApps > 0 && <div className="flex items-center justify-between p-3 bg-amber-50/50 border border-amber-100/35 rounded-xl text-xs font-bold text-amber-800">
                      <span>Applications Pending</span>
                      <span className="font-black text-amber-600">{pendingApps}</span>
                    </div>}
                  {approvedApps > 0 && <div className="flex items-center justify-between p-3 bg-emerald-50/50 border border-emerald-100/35 rounded-xl text-xs font-bold text-emerald-800">
                      <span>Applications Approved</span>
                      <span className="font-black text-emerald-600">{approvedApps}</span>
                    </div>}
                  {(approvedApps > 0 || totalTeaching > 0) && <Link to="/teacher-panel" className="flex items-center justify-center gap-2 w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs transition-all shadow-md"> <Users className="w-4 h-4" /> Open Teacher Panel
                    </Link>}
                </div>
              </motion.div>}
    ...
    className="border border-slate-200 rounded-[2rem] shadow-sm overflow-hidden"
  >
                <div className="flex items-center justify-between px-5 pt-5 pb-3 "> <h3 className="font-black text-slate-800 text-xs sm:text-sm flex items-center gap-2">
                ...
                <div className="max-h-60 overflow-y-auto divide-y divide-slate-100 custom-scrollbar"> {notifications.slice(0, 10).map((n) => <div key={n.id} className="flex items-start gap-3 px-5 py-3.5 hover:/80 transition-colors ">
                      <div className="w-8 h-8 rounded-xl bg-amber-50 text-amber-600 border border-amber-100 flex items-center justify-center shrink-0 text-xs shadow-inner"> {n.type === "schedule" ? <Calendar className="w-4 h-4" /> : <Bell className="w-4 h-4" />}
                      </div>
                      <div className="flex-1 min-w-0"> <h4 className="text-xs font-black text-slate-850">{n.title}</h4>
                        <p className="text-[10px] text-slate-500 mt-0.5 leading-relaxed">{n.message}</p> <span className="text-[8px] font-bold text-slate-400 mt-1 block">
