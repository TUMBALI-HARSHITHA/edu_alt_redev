...
${courses.slice(0, 30).join("\n")}`);
  ...
  const openWithMode = useCallback((targetMode) => {
    setMode(targetMode);
    setMessages([]);
    setIsOpen(true);
  }, []);
...
User question/request: ${msg}`;
  ...
  >
      ...
      {isOpen && <div className="fixed bottom-24 right-6 z-50 w-[380px] max-w-[calc(100vw-2rem)] bg-white rounded-3xl border border-slate-200 shadow-2xl flex flex-col overflow-hidden max-h-[600px]">
          {!currentUser ? <div className="p-8 flex flex-col items-center justify-center text-center min-h-[300px]">
  ...
  >
                  ...
                  {showModePicker && <div className="absolute top-full left-0 mt-1 bg-white rounded-xl border border-slate-200 shadow-xl p-1 z-10 min-w-[200px]">
                      {MODES.map((m) => <button
    key={m.id}
    onClick={() => switchMode(m.id)}
    className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-bold transition-colors ${mode === m.id ? "bg-emerald-50 text-emerald-600 " : "text-slate-600 hover:bg-slate-50 :hover:bg-slate-700"}`}
  >
                          {m.icon}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[300px] max-h-[400px] custom-scrollbar">
            {messages.map((msg, i) => <div key={i}>
                <div className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed ${msg.role === "user" ? "bg-emerald-500 text-white rounded-br-md" : "bg-slate-100 text-slate-700 rounded-bl-md"}`}>
                    <p className="whitespace-pre-wrap">{msg.content}</p>
                  </div>
                </div>
                {msg.role === "assistant" && !loading && i === messages.length - 1 && <div className="flex justify-start ml-2 mt-1">
                    <button
